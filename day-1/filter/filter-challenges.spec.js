...
describe('using filter()', () => {
    ...
    describe('getOldCats()', () => {
        it('returns empty array when cat is older than 8', () => {
            const cats = [{ name: 'Bean', age: 9 }];
            const actual = getOldCats(cats);
            const expected = [];
            expect(actual).to.eql(expected);
        });
        it('returns array of 1 cat when younger than 8', () => {
            const cats = [{ name: 'Shnitz', age: 4 }];
            const actual = getOldCats(cats);
            const expected = [{ name: 'Shnitz', age: 4 }];
            expect(actual).to.eql(expected);
        });
